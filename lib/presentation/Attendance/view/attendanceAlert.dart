...
class _AttendanceAlertViewState extends State<AttendanceAlertView> {
                            ...
                            children: [
                              TextFormField(
                                  ...
                                  onTap: () async {
                                    ...
                                    setState(() {
                                      String _newDate = DateFormat('yMMMM')
                                          .format(newDate)
                                          .toString();
                                      _startDate = _newDate;
                                    });
  ...
  Future<List<UserAttendanceAlert>?> getApiAttendance(String date) async {
    ...
    a = await userAttend
        .map((data) => UserAttendanceAlert.fromJson(data))
        .toList();
    List<UserAttendanceAlert>? b =
        List<UserAttendanceAlert>.from(a as Iterable);
    return b;
    // }
    //return null ;
  }
  ...
  Future<Widget> _createAttendanceTable(
      List<UserAttendanceAlert>? attendance) async {
    if (attendance?.isEmpty == false) {
      return DataTable(
        headingRowColor: MaterialStateColor.resolveWith(
            (states) => colorManager.lightprimary),
  ...
  List<DataColumn> _createColumns() {
    return [
      ...
      DataColumn(
          label: Text(
        AppStrings.Alert_Type.tr(),
        style: TextStyle(color: colorManager.white),
      )),
      DataColumn(
          label: Text(
        AppStrings.Alert_Handling.tr(),
        style: TextStyle(color: colorManager.white),
      )),
      DataColumn(
          label: Text(
  ...
  List<DataRow> _createRows(List<UserAttendanceAlert>? attendances) {
    return attendances!
        .map((attendanceItem) => DataRow(cells: [
              DataCell(Text(DateFormat('MMMMd')
                  .format(DateTime.parse(attendanceItem.date.toString()))
                  .toString())),
