...
class _AttendanceViewState extends State<AttendanceView> {
  ...
  final TextEditingController _FromDateEditingController =
      TextEditingController();
  final TextEditingController _ToDateEditingController =
      TextEditingController();
                        ...
                        rows: attendance
                            .map((item) => DataRow(cells: [
                                  ...
                                  DataCell(Text(DateFormat('dd-MM-yyyy')
                                      .format(item.actiontime as DateTime)
                                      .toString()))
                                ]))
                            .toList()),
                  ),
              ...
              children: [
                ...
                Expanded(
                  child: Container(
                      ...
                      height: 30,
                      child: Text(AppStrings.to.tr() + ' : ' + _endDate,
                          style: TextStyle(
                              fontSize: 13, fontWeight: FontWeight.bold))),
                )
              ]),
  ...
  Future<List<UserAttendance>?> getApiAttendance() async {
    ...
    var uri = Uri.parse(Constants.attendanceUrl +
        _startDate +
        'T11:41:47.053Z&toDate=' +
        _endDate +
        'T11:41:47.053Z');
    ...
    if (responseData['data'] != null) {
      ...
      a = await userAttend
          .map((data) => UserAttendance.fromJson(data))
          .toList();
  ...
  Widget _createAttendanceTable1(List<UserAttendance>? attendance) {
    if (attendance?.isEmpty == false) {
      return DataTable(
        headingRowColor: MaterialStateColor.resolveWith(
            (states) => colorManager.lightprimary),
  ...
  Future<Widget> _createAttendanceTable(
      List<UserAttendance>? attendance) async {
    if (attendance?.isEmpty == false) {
      return DataTable(
        headingRowColor: MaterialStateColor.resolveWith(
            (states) => colorManager.lightprimary),
  ...
  List<DataRow> _createRows(List<UserAttendance>? attendances) {
    return attendances!
        .map((attendanceItem) => DataRow(cells: [
              DataCell(Text((attendanceItem.action).toString())),
              DataCell(Text((DateFormat.Hm()
                      .format(DateTime.parse(attendanceItem.actiontime)))
                  .toString())),
              DataCell(Text((DateFormat.MEd()
                      .format(DateTime.parse(attendanceItem.actiontime)))
                  .toString()))
            ]))
        .toList();
          ...
          switch (snapshot.connectionState) {
            ...
            default:
              if (snapshot.hasError)
                ...
                return Center(child: _createAttendanceTable1(attendanceData));
              }
          }
          return const Text('');
        });
