...
class ReviewedAdmin extends StatefulWidget implements NavigationStates
{
  const ReviewedAdmin ({Key? key}) : super(key: key);
  @override
  State<ReviewedAdmin> createState() => _ReviewedAdminViewState();
}

class _ReviewedAdminViewState extends State<ReviewedAdmin>with TickerProviderStateMixin {

  final AppPreferences _appPreferences = instance<AppPreferences>();
  String? userId;
  List<ReviewedAdminModel>? AdminsData;
  ...
  Widget AdminTable(){
    return
      FutureBuilder(
          future:getReviewedAdminRequests(),
  ...
  Widget _createAdminsTable(List<ReviewedAdminModel> admin) {
  ...
  List<DataRow> _createRows(List<ReviewedAdminModel> admin) {
    ...
    return admin
        ...
        DataRow(cells: [
          ...
          DataCell(Column(mainAxisSize: MainAxisSize.min, children: [Text(DateFormat('dd-MM-yyyy').format(DateTime.tryParse(admin.date!)!).toString()),])),
  ...
  Future <List<ReviewedAdminModel>?> getReviewedAdminRequests() async {
    List<ReviewedAdminModel>? a;
    userId = await _appPreferences.getUserToken();
    var response = await http.get(
        Uri.parse(Constants.getReviewedAdmin), headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8','userId':userId!});
    ...
    if(responseData!=null) {
      ...
      a = ( Admins.map((jsonData) =>
          ReviewedAdminModel.fromJson(jsonData)).toList());
      List<ReviewedAdminModel>? b = List<ReviewedAdminModel>.from(a as Iterable);
      AdminsData = b;
      return AdminsData;
    } return null;
  }
