...
class ReviewFinancial extends StatefulWidget implements NavigationStates
{
  const ReviewFinancial ({Key? key}) : super(key: key);
  @override
  State<ReviewFinancial> createState() => _ReviewFinancialiewState();


}

class _ReviewFinancialiewState extends State<ReviewFinancial>with TickerProviderStateMixin {

  final AppPreferences _appPreferences = instance<AppPreferences>();
  String? userId;
  List<ReviewFinancialModel>? FinancialsData;
  ...
  Widget FinancialTable(){
    return
      FutureBuilder(
          future:getReviewFinancialRequests(),
  ...
  Widget _createFinancialsTable(List<ReviewFinancialModel> Financial) {
  ...
  List<DataRow> _createRows(List<ReviewFinancialModel> financial) {
  ...
  Future <List<ReviewFinancialModel>?> getReviewFinancialRequests() async {
    List<ReviewFinancialModel>? a;
    userId = await _appPreferences.getUserToken();
    var response = await http.get(
        Uri.parse(Constants.getReviewFinancial), headers: <String, String>{'Content-Type': 'application/json; charset=UTF-8','userId':userId!});
    ...
    if(responseData!=null) {
      ...
      a = (financials.map((jsonData) =>
          ReviewFinancialModel.fromJson(jsonData)).toList());
         List<ReviewFinancialModel>? b = List<ReviewFinancialModel>.from(a as Iterable);
      FinancialsData = b;
